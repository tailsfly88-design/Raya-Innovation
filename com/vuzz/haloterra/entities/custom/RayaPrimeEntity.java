...
implements IFlyingAnimal {
    ...
    public ActionResultType func_230254_b_(PlayerEntity player, Hand hand) {
        if (player == this.owner) {
            if (player.func_225608_bj_()) {
                ...
                if (this.owner == player && this.getEnergy() >= 0.0f && nbt.func_74767_n("canUseEnergy")) {
                    INamedContainerProvider containerProvider = this.createContainerProvider(player.func_130014_f_(), player.func_233580_cy_());
                    NetworkHooks.openGui((ServerPlayerEntity)((ServerPlayerEntity)player), (INamedContainerProvider)containerProvider, (BlockPos)this.func_233580_cy_());
                }
            }
        }
    ...
    private void returnAsImplant() {
        ...
        if (this.owner instanceof PlayerEntity) {
            ...
            ItemStack item = new ItemStack((IItemProvider)ModItems.INACTIVE_IMPLANT.get());
            if (player.func_213365_e(item)) {
                item.func_77982_d(nbt);
                player.func_191521_c(item);
                player.getPersistentData().func_74757_a("hasraya", false);
                this.func_70106_y();
            } else {
                this.func_70106_y();
                item.func_77982_d(nbt);
    ...
    public void func_70071_h_() {
        ...
        if (this.func_130014_f_().field_72995_K) {
            nbt.func_74768_a("anim", this.anim);
        } else {
            this.anim = nbt.func_74762_e("anim");
            if (Minecraft.func_71410_x().field_71441_e.func_73045_a(this.func_145782_y()) != null) {
                Minecraft.func_71410_x().field_71441_e.func_73045_a(this.func_145782_y()).getPersistentData().func_74768_a("anim", this.anim);
            }
        }
        if (this.owner.func_70644_a((Effect)ModEffects.HYBERNATION.get())) {
            PlayerEntity player = (PlayerEntity)this.owner;
            this.owner.func_145747_a((ITextComponent)new TranslationTextComponent("message.haloterra.disabling"), Util.field_240973_b_);
        ...
        if (distanceBetween > 60.0) {
            ...
            this.func_70661_as().func_75492_a(this.func_70902_q().func_226277_ct_() - 2.0, this.func_70902_q().func_226280_cw_(), this.func_70902_q().func_226281_cx_() - 2.0, 1.3);
        } else {
            this.func_213293_j(0.0, this.clamp((this.func_70902_q().func_226280_cw_() - this.func_226278_cu_() - 0.4) * 1.1, -0.2, 0.2) * 1.1 + Math.sin(this.ticksPast / 4) / 5.0, 0.0);
            this.func_70661_as().func_75499_g();
        }
        PlayerEntity player = (PlayerEntity)this.owner;
        if (this.ticksPast - this.lastHungerCheck >= 600 && player.func_71024_bL().func_75116_a() < 10) {
            this.owner.func_145747_a((ITextComponent)new TranslationTextComponent("message.haloterra.lowfood"), Util.field_240973_b_);
            this.lastHungerCheck = this.ticksPast;
        }
        ItemStack stackForCheck = player.func_184614_ca();
        if (this.ticksPast - this.lastDurabilityCheck >= 300 && stackForCheck.func_77952_i() * 100 / (stackForCheck.func_77958_k() + 1) >= 90) {
            this.owner.func_145747_a((ITextComponent)new TranslationTextComponent("message.haloterra.lowdurability"), Util.field_240973_b_);
            this.lastDurabilityCheck = this.ticksPast;
        }
        if (this.ticksPast - this.lastHealthCheck >= 600 && player.func_110143_aJ() <= 4.0f) {
            player.func_195064_c(new EffectInstance(Effects.field_76424_c, 300, 1));
            this.owner.func_145747_a((ITextComponent)new TranslationTextComponent("message.haloterra.adrenaline"), Util.field_240973_b_);
